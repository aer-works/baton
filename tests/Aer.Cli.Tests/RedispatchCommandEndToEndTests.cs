...
/// <summary>
/// <c>aer redispatch &lt;room-dir&gt;</c> end to end (#1441): a TERMINAL room produced by a real
/// <c>aer dispatch</c> is redispatched into a fresh room, driven through the exact pump <c>aer
/// dispatch</c>/<c>aer run</c> share, so the inherited binding is exercised for real rather than just
...
public sealed class RedispatchCommandEndToEndTests : IDisposable
{
    ...
    public async Task Lineage_is_recorded_naming_the_parent_room_and_its_execution_id()
    {
        ...
        try
        {
            ...
            var markerPath = Path.Combine(childRoom, ".aer", AerPaths.RoomMetadataFileName);
    ...
    public async Task Redispatching_a_composed_template_room_is_refused()
    {
        ...
        try
        {
            ...
            await WorkerBindingConfigWriter.SaveToFileAsync(
                multiWorkerBindings, AerPaths.RoomBindingsFile(parentRoom), TestContext.Current.CancellationToken);
