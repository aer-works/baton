...
/// <summary>
/// <c>aer dispatch &lt;template&gt;</c> end to end (rung-3, #920): a shipped multi-phase template is
/// resolved, composed into a DAG, and driven through the exact pump <c>aer run</c> uses — and when it
...
public sealed class DispatchTemplateEndToEndTests : IDisposable
{
    ...
    // middle rung ({AerPaths.Root}/*.json) and would silently read an operator's local override on a
    ...
    public async Task Dispatching_the_shipped_template_composes_runs_and_injects_the_capture_base_ref()
    {
        ...
        try
        {
            ...
            // schema is Aer.Flow.Domain.ReviewVerdict; this is the smallest document it accepts.
