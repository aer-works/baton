...
public class DeniedToolChannelTests
{
    ...
    public void A_tagged_empty_list_allows_on_both_vendors()
    {
        ...
        // escape hatch — which is the ordinary `aer run` shape. Denying here breaks every such worker.
    ...
    {
        // Aer.Adapters cannot reference Aer.Cli, so the tag is a literal on each side. This is the one
        // test that sees both, and it is what stops a rename on one side silently denying everything.
        var contract = new Aer.Flow.Domain.WorkerContract("w", [], [], []);
