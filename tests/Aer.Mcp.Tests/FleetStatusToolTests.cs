...
public sealed class FleetStatusToolTests : IDisposable
{
    ...
    {
        _tempHome = Path.Combine(Path.GetTempPath(), $"aer-fleet-test-home-{Guid.NewGuid():N}");
    ...
    public async Task Enumeration_IncludesExtraRoots_AndDiscoversRoomsAcrossRoots()
    {
        ...
        var extraRoot = Path.Combine(Path.GetTempPath(), $"aer-fleet-test-extra-{Guid.NewGuid():N}");
