...
public class CoreDispatcherTests
{
    ...
    public void AssembleChildEnvironment_orders_inherited_then_computed_then_target_and_drops_passthrough()
    {
        var request = MakeRequest(
        [
            new EnvironmentVariable.AerComputed("AER_COMPUTED_X", "cval"),
        ...
        Assert.Contains(("AER_COMPUTED_X", "cval"), environment);
        ...
        Assert.True(names.IndexOf("PATH") < names.IndexOf("AER_COMPUTED_X"));
        Assert.True(names.IndexOf("AER_COMPUTED_X") < names.IndexOf("TARGET_Y"));
    }
