...
public class FlowEventLogJsonTests
{
    ...
    /// aer-core's own ABI numbering evolves". Storing it as an ordinal made that claim false by
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (var parameter in type.GetConstructors().SelectMany(c => c.GetParameters()))
            {
                foreach (var candidate in Unwrap(parameter.ParameterType))
                {
                    ...
                    else if (candidate.Namespace?.StartsWith("Baton.Flow", StringComparison.Ordinal) == true)
                    {
