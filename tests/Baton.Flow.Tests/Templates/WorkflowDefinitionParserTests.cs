...
namespace Baton.Flow.Tests.Templates;
...
public class WorkflowDefinitionParserTests
{
    ...
    public void Every_converter_reachable_from_the_template_graph_is_known_to_the_prefix_list()
    {
        ...
        {
            "Baton.Flow.Domain.BackoffPolicyJsonConverter",
            "Baton.Flow.Domain.StepId+Converter",
            "Baton.Flow.Domain.WorkflowTemplateId+Converter",
        };
        ...
        void Walk(Type type)
        {
            ...
            if (type.Namespace?.StartsWith("Baton.Flow", StringComparison.Ordinal) != true
                || !visited.Add(type))
            {
                return;
            }
    ...
    [Fact]
    public async Task LoadFromFileAsync_on_a_missing_file_without_json_extension_suggests_baton_dispatch()
    {
        ...
        Assert.Contains("does not exist", ex.Message);
        Assert.Contains("'baton run' takes a workflow FILE; built-in templates are used via 'baton dispatch <role>'", ex.TryInvocation, StringComparison.Ordinal);
    }
}
