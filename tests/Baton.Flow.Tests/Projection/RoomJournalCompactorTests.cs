...
public class RoomJournalCompactorTests
{
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "baton_compactor_test_" + Guid.NewGuid().ToString("n"));
    ...
    public async Task CompactAsync_refuses_while_the_room_lock_is_held_by_someone_else()
    {
        ...
        using (Baton.Flow.Concurrency.ConcurrencyGuard.AcquireRoomEvents(roomDir, "test holder"))
        {
        ...
        using (Baton.Flow.Concurrency.ConcurrencyGuard.Acquire(roomDir, "test flow holder"))
        {
            Assert.True(await RoomJournalCompactor.CompactAsync(roomDir, TestContext.Current.CancellationToken));
        }
    ...
    {
        var resolvedDir = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "baton_wf_resolved_" + Guid.NewGuid().ToString("n")));
        ...
        try
        {
            ...
            var freshDir = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "baton_wf_fresh_" + Guid.NewGuid().ToString("n")));
