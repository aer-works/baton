...
/// <summary>
/// #1848 review: <b>production dispatch must use the real runway evaluator.</b>
/// <c>DispatchCommand.ExecuteAsync</c>'s <c>evaluateRunway</c> parameter is a public optional seam that
/// exists so a test can drive the gate's arms without a harvested snapshot; thirteen tests pass their
/// own. Nothing else made it impossible for a production call site to pass one too — and a seam that
/// silently admits every dispatch is indistinguishable from a working gate until the month's usage bill
/// says otherwise. This is what makes that a build failure instead of a review's job.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <list type="number">
/// <item>A POSITIONAL fifth argument — <c>ExecuteAsync(options, adapters, token, workspace, evaluator)</c>
/// — carries no <c>evaluateRunway:</c> text and is invisible here. Every call site in the tree names the
/// argument; this scan cannot enforce that it keeps doing so.</item>
/// <item>An evaluator reaching the call through a variable declared elsewhere in the file, or through a
/// <c>DispatchOptions</c>-shaped indirection, is out of the statement this reads.</item>
/// <item>It says nothing about what the production evaluator DOES. That
/// <c>CreateDiskRunwayEvaluatorAsync</c> reads the operator's thresholds and the harvested snapshot is
/// pinned by <c>Baton.Vendors.Tests.RunwayGateTests</c> and
/// <c>Baton.Vendors.Tests.RunwayHoldSettingsTests</c>, not by a file scan.</item>
/// </list>
/// </remarks>
public class ProductionRunwayGateSeamTests
{
    /// <summary>
    /// Production call sites permitted to pass their own <c>evaluateRunway</c>, with why. Empty by
    /// design — see the remarks above before adding one.
    /// </summary>
    ...
    private const string SeamMarker = "evaluateRunway";
    ...
    public void No_production_call_site_supplies_its_own_runway_evaluator()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            "A production call to DispatchCommand.ExecuteAsync passes its own 'evaluateRunway':\n  "
            + string.Join("\n  ", offenders)
            + "\n\nThat seam exists for tests. Production must leave it null so dispatch reads the "
            + "operator's thresholds and the harvested snapshot (spec/baton.md §7, \"Runway hold "
            + "(#1848)\"). If a production caller really needs its own, that is a spec change first — "
            + "then add it to ApprovedEvaluatorOverrides with the reason.");
    }
