...
public class VendorSpawnGateTests
{
    ...
    private static readonly Dictionary<string, string> ApprovedSpawnSites = new()
    {
        ...
        ["src/Baton.Cli/CostLedgerSettlementMetadata.cs"] = "#1901: read-only local git branch/ancestry/diff probes used to enrich settled cost rows. Every git and injected forge-runner call has its own 20-second bound linked to settlement cancellation; it never invokes a model vendor process.",
