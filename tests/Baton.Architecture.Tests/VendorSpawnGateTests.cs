...
public class VendorSpawnGateTests
{
    ...
    private static readonly Dictionary<string, string> ApprovedSpawnSites = new()
    {
        ...
        ["src/Baton.Cli/WatchNotifier.cs"] = "#1488: spawns the platform shell ('cmd /c' on Windows, 'sh -c' on Unix) to run an OPERATOR-authored '--notify <command>' target once, when a watched room reaches Terminal — a webhook script, curl, an ntfy call, or similar wake-up command the operator wrote, never a vendor CLI and never anything AER derives arguments for. No dispatch, no tool call, no PreToolUse surface: the payload only ever reaches the spawned process via stdin/BATON_WATCH_EVENT, never interpolated into the command line, and the command itself is never claude/agy or an AER-constructed invocation of either.",
    };
