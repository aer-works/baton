...
public class VendorSpawnGateTests
{
    ...
    private static readonly Dictionary<string, string> ApprovedSpawnSites = new()
    {
        ...
        ["src/Baton.Cli/CostLedgerSettlementMetadata.cs"] = "#1901: read-only local git branch/ancestry/diff probes used to enrich settled cost rows. It never invokes a vendor process, and its forge query goes through the separately reviewed IGhCliRunner.",
