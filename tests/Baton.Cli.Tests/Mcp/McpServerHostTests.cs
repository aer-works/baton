...
public class McpServerHostTests
{
    ...
    public async Task Initialize_ReturnsProtocolVersionAndServerInfo()
    {
        ...
        Assert.Equal("baton-mcp-host-test", (string)responses[0]["result"]!["serverInfo"]!["name"]!);
    }
    ...
    {
        var captureFile = Path.Combine(Path.GetTempPath(), $"baton-mcp-test-{Guid.NewGuid():N}.json");
        try
        {
            var host = new McpServerHost("baton-mcp-host-test", "1.0.0", [new YieldTool(captureFile)]);
    ...
    public async Task ToolsCall_ToolThrows_ReturnsIsErrorResult_LoopSurvives()
    {
        ...
        var host = new McpServerHost("baton-mcp-host-test", "1.0.0", [throwingTool]);
        var input = new StringReader(
    ...
    {
        var host = new McpServerHost("baton-mcp-host-test", "1.0.0", [new AnnotatedTool(), new ThrowingTool()]);
    ...
    {
        var host = new McpServerHost("baton-mcp-host-test", "1.0.0", [new BadAnnotationsTool(), new AnnotatedTool()]);
        var input = new StringReader(
            "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}\n"
            + "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/list\",\"params\":{}}\n");
        var output = new StringWriter();
