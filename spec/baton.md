...
| `issue`, `pr` | Settle-time work join from the branch-delivering worker: `issue` is the leading issue number in its live workspace branch and `pr` is the existing PR returned for that branch. Only after that workspace has disappeared may settlement use the branch stamped at dispatch; a branch with no PR writes `issue` only. |
| `deliverySource` | `workspace-head` when the join and diff came from the delivering worker's live workspace HEAD, or `dispatch-stamped-branch` when its workspace was gone and settlement used the dispatch stamp plus the source repository's remote-tracking ref. Absent when neither source could be established. |
...
| `verdict`, `findingsHigh`, `findingsMedium`, `findingsLow` | A valid execution-scoped `verdict.json` whose findings all explicitly carry severity and status: `APPROVE` when it has no confirmed findings, otherwise `BLOCK`; the three counts include confirmed findings only. If any finding omits either classification, all four fields stay absent instead of inheriting enum defaults; zero is written only for an explicitly classifiable verdict. |
| `reviewedPr`, `reviewedHead` | Components parsed independently from `verdict.json`'s required, otherwise-verbatim `reviewedRef`: `reviewedPr` requires an explicit PR shape such as `#2001`, `PR #2001`, or `pull/2001`; `reviewedHead` requires exactly 40 hexadecimal characters, either alone or after `@`/`head:`. Bare numbers, issue references, and branch-like hexadecimal names yield neither. |
| `filesChanged`, `additions`, `deletions`, `testFilesChanged` | For the last successful `deliversBranch` execution whose HEAD is confirmed on `origin/<branch>`: one `git diff --numstat origin/main...HEAD` supplies all four values, including test paths. Empty successful output is the real 0/0/0 diff; any non-empty malformed output leaves all four absent, as do an unconfirmed push or a failed probe. |
