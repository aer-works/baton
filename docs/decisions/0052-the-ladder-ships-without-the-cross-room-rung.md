...
is the only rung whose permission is not room-scoped, and building it requires a decision the corpus does not
make, namely *where a cross-room standing permission is stored*.
...
when it returns it is a *project*-scoped permission, not an account-wide one.**
...
1. **There is no scope to hang it on.** 0004's model is project ∩ room ∩ step, always narrowing. There
   ...
   *additive* standing permission that pre-approves a command.
...
2. **Account-wide is the wrong home.** The only account-wide precedent is skills
   ...
   standing permission applies with no per-room act at all. A per-command "allow everywhere" list is a
...
3. **Project scope is the right home, and it is cheaper.** What a person who wants "any rm in any room"
   ...
   already keys AER-side per-project state by normalized absolute path. A project-scoped standing permission
...
| 0034's project store is a subtractive ceiling, not an additive permission | **the record** — [0034](0034-project-permission-ceiling-lives-in-aers-own-config.md) | the project store could hold the permission as-is today and the "needs a store" premise weakens |
...
(`bindings.json`), one lifecycle (the permission dies with its room, [0009](0009-session-lifecycle-and-retention.md)),
...
honesty clause) that "any room" is not among them; and, when the project-scoped standing-permission store is
built on 0034's per-project keying, return the rung there — as a project permission, never an account one.
