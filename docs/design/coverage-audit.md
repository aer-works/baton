...
| Call | Source | Status |
|---|---|---|
| Vendor / model / effort are three choices, all on the chip | 04 | ◐ **partial** — [0017](../decisions/0017-vendor-model-effort-are-three-choices.md) is landed as the three-axis engine model, but today's single-worker chip (`ChatHeaderView.axaml:70-72`, `Chat.WorkerChipText`/`Chat.WorkerModelText`) shows only vendor and model — **effort renders on no chip anywhere** (zero hits for `Effort` across `Aer.Ui`/`Aer.Ui.Core` outside a doc comment). [0058](../decisions/0058-model-and-effort-are-marks-not-words.md) confirms the gap and assigns the fix to the multi-worker workflow-room chip (`#1318`, open — `#511`'s composite-fill blocker is itself now closed). |
| **Effort is named by behaviour** — quick / standard / careful / exhaustive — never a token budget or a vendor's flag name. | 04, 05 | **→** the rule is now **landed** as [0023](../decisions/0023-effort-and-models-are-named-by-behaviour.md) (accepted 2026-07-24; `#472` confirmed both CLIs expose a real `--effort`), with [0058](../decisions/0058-model-and-effort-are-marks-not-words.md) layering presentation (achromatic marks, not words) on top. The mapping is measured and registered (`docs/vendor-capabilities.md`, `vendor-verify` sentinels). Implementation is absent: `AgyWorkerAdapter.cs:939-940` and `ClaudeWorkerAdapter.cs:283-286` still pass each vendor's raw effort string straight through; no `quick`/`standard`/`careful`/`exhaustive` string exists in `src/`. Chip rendering → `#1318`; `#498` (the vocabulary picker itself) closed noting that remit "remains unbuilt and unsliced." |
| **Models are offered by purpose** — deep / balanced / fast. Nobody should need this month's model string. | 04 | **→** the rule is the same [0023](../decisions/0023-effort-and-models-are-named-by-behaviour.md); [0058](../decisions/0058-model-and-effort-are-marks-not-words.md) assigns it to the chip's depth mark. Unlike effort, the **mapping itself is not yet measured** — `#1330` (open) states plainly that no vendor→tier table exists in code, docs, or a decision. Implementation absent: no `deep`/`balanced`/`fast` string anywhere in `Aer.Ui`/`Aer.Ui.Core`; `WorkerModelText` shows the raw model string. Chip rendering → `#1318`. |
...
| **Artifacts are files** — vendor-neutral, versioned, attributed, explicitly attached, diffable between vendors. | 03, 07 | **→** the rule is now **landed** as [0021](../decisions/0021-artifacts-are-files.md) (accepted 2026-07-24 — this row previously said "new decision record," not yet written; it has been). Implementation is absent: `ArtifactManager.cs` computes `execution_{id}/` directory paths and env vars only — no attribution, no version concept; `ArtifactFileViewModel` (`RoomStepViewModels.cs:490-505`) is a bare `(Label, FilePath)`. `#377` (under `#751`) covers a diff *viewer*, not this model, and closed as grouped rather than shipped. No issue tracks building the model itself. |
...
| **Documents stay, plumbing goes.** One file list; the only distinction is "in your project" or not. Execution directories are never surfaced. | 04 | **→ same record**, now [0021](../decisions/0021-artifacts-are-files.md) §2. Absent — the file listings that exist (`ArtifactFileViewModel`, `ConversationRefViewModel`) are all per-execution; no unified cross-room list and no project-vs-execution distinction exist. |
...
[0021](../decisions/0021-artifacts-are-files.md) (2026-07-24). Its implementation is not routed: no
issue tracks building the versioned/attributed file model itself (`#377` covers only a diff viewer over
something else, and is closed-as-grouped rather than built).
