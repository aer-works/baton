...
Waiting on another process's lock | Reads as a wait, never as an error and never as generic working: names the holder process and how long it has held the lock. Corrected 2026-08-16 (#1299, Fable's ruling): room identity is directory-keyed, so two rooms cannot share a folder as distinct rooms — the collision is always a foreign process (a bare aer run pump, the sweep, a second Baton instance), never another room, so there is nothing to link to and no duplicate-room warning to show; the wait itself, shown the instant it bites, is the knowing choice. *(#480 (grouped under #752), ratified 2026-08-04 on #495, corrected 2026-08-16 on #1299)* |
...
A wait names its holder (added 2026-08-04, corrected 2026-08-16 on #1299) Waiting on another process's lock is its own state, never generic working: the holder process is named, with how long it has held the lock. Room identity is directory-keyed, so the holder is never another room and there is nothing to link to — naming it is the discoverable part, not a "go close the other room" navigation.
