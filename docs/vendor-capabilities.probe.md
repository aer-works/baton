| | `agy` 1.1.23 *(carried, not re-probed)* | `claude` 2.1.258 (Claude Code) *(carried, not re-probed)* | `codex` codex-cli 0.153.2 |
|---|---|---|---|
| plan usage & reset | not found on: --help, subcommand list, in-session slash command | **`/usage` — 2% used, with reset times** | **`account/rateLimits/read` — 13% used, resets 2026-09-11T16:37:48.0000000+00:00, 0% used, resets 2026-09-05T01:52:17.0000000+00:00, 0% used, resets 2026-09-11T20:52:17.0000000+00:00** |
| per-turn cost | not found on: structured output stream, --help | **`total_cost_usd` in every result event** | not found on: structured output stream |
| structured output | **`--output-format stream-json`** | **`--output-format stream-json --verbose`** | **`codex exec --json` JSONL** |
| --permission-prompt-tool | not found on: --help, stderr, flag acceptance vs. a control flag, structured output stream | **`--permission-prompt-tool <mcp-tool>` — consulted for permission decisions** | not found on: exec --help |
| effort | `--effort` — low|medium|high *(inspected, not run)* | `--effort` — low, medium, high, xhigh, max *(inspected, not run)* | **model-specific reasoning efforts from `model/list`** |
