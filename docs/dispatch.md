...
| `--override-runway <reason>` | Dispatch anyway when the vendor's runway hold would refuse this dispatch (#1848) — the only bypass, and the reason is mandatory (a blank one is a parse error). Applies to a role **and** a template dispatch, unlike `--timeout`/`--output` above: both admit new vendor spend. The reason, the vendor, and the counters the gate read are written to the room's `bindings.json` as `runwayOverride`, and — when a hold was actually bypassed — onto the cost ledger row as `runwayOverrideReason` (#1849). Passed when the gate would have admitted anyway, it is still recorded, as `used: false`. Full contract in `spec/baton.md` §7, "Runway hold (#1848)". |
