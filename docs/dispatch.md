...
| `--continue <room-dir>` | Rehire the worker that ran in this terminal room for a follow-on brief, resuming its vendor session instead of starting cold (#1381) — the general manual counterpart to #1373's automatic retry-with-continuation. Q1 scope: **claude only** today (both the veteran's recorded adapter and this dispatch's own resolved one must be `claude`); agy rehire is gated on its own resume measurement. Refuses loudly rather than silently going cold when the named room does not exist, dispatched more than one worker, has no vendor session id recorded (an ordinary dispatch mints none automatically — see `spec/baton.md` §3), or has not yet reached a terminal state. Role dispatch only — refused up front on a template dispatch, the same way `--verify` is. Full contract in `spec/baton.md` §3. |
