# `baton-agy-loop` — dispatch one AER workflow step, read back its output

```
pixi run baton-dispatch -- --list-templates

pixi run baton-dispatch -- \
...
Prints the produced output file's content to stdout. On failure, prints `baton run`'s own output plus
...
- `--room-dir` must be absolute. A relative one resolves against the CLI's own cwd, but `agy` runs
  ...
  `BATON_OUTPUT_DIR` the dispatched process resolves against the wrong root — the run exits 0, the
...
timeout as a set. Run `pixi run baton-dispatch -- --list-templates` for what each one is
...
- **A read-only dispatch is refused before it can spend**, because a worker satisfies its
  `ProducedOutputs` contract only by writing into `BATON_OUTPUT_DIR`
...
resolved grant, and exits **without calling `baton run`**. Spends nothing, and does not need a built
`Baton.Cli.exe` — it reports its absence instead of failing, which is what lets
...
`--run-shell-commands` without `--network-access` is refused client-side, before ever calling `baton
