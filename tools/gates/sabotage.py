...
def _sabotage_diff_shape_selftest() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        # widened-path arms (l)-(p) assert those protected-tooling edits fail, so a neutered
        # predicate must go red (#1744: pixi.toml itself no longer routes through this function,
        # so those arms -- not (d) -- are what a neutered predicate now flips).
        mutated = src.replace(
