...
await Baton.Daemon.DaemonHost.RunDaemonAsync(args);
...
namespace Baton.Daemon
{
    public static class DaemonHost
    {
        ...
        /// <summary>Test-only seam (Baton.Daemon.Tests, via <c>InternalsVisibleTo</c>): <paramref name="onHostBuilt"/>
        ...
        internal static async Task RunDaemonAsync(string[] args, Action<IHost>? onHostBuilt)
        {
            ...
            if (!noMutex)
            {
                ...
                if (!createdNew)
                {
                    Console.WriteLine("Another instance of Baton.Daemon is already running.");
                    mutex.Dispose();
                    return;
                }
            }
