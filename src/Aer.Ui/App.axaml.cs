...
public partial class App : Application
{
    ...
    private void Report(Exception exception)
    {
        ...
        if (ErrorSurface is { } surface)
        {
            ...
            // with every routine status update (MainWindow's own writers, RoomClient's mutation
            // status), so it can be overwritten by the next ordinary message before it is read.
            // Where the detail persists has to be part of what the person is told.
            var message = $"{PlainLanguage.ForUnexpectedAppError()}: {exception.Message} — written to {AppUnhandledExceptionSink.LogPath}";
