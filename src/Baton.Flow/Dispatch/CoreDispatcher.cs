...
/// <c>Baton.Flow</c> entirely, so the caller supplies it explicitly rather than the dispatcher
...
/// 2 keeps vendor quirks inside <c>Baton.Vendors</c> rather than letting <c>Baton.Flow</c> know the
...
/// <c>CLAUDE.md</c> requires: the only place in <c>Baton.Flow</c> that touches <c>Baton.Core</c>
/// directly.
/// </summary>
