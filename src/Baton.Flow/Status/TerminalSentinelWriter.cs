...
public static class TerminalSentinelWriter
{
    ...
    /// and sometimes before <c>snapshot.json</c> — ever existed, so there is no <see cref="Baton.Flow.Domain.FlowState"/>
    ...
    /// refusing <see cref="Baton.Flow.BatonFlowException.TryInvocation"/> through to the same field a
