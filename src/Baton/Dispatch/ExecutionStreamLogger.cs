...
public sealed class ExecutionStreamLogger
{
    ...
    /// size it for <c>pruned[].bytes</c> — and it deliberately sums unfiltered rather than applying
    /// this filter: that field answers "how much did pruning reclaim", and stream logs were reclaimed
    /// too, so their bytes count. <c>Baton.Architecture.Tests.ExecutionOutputDirectoryListingTests</c>
