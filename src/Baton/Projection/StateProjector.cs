    ...
    private static void ApplyEvent(FlowEvent flowEvent, ProjectionCheckpointState state)
    {
        switch (flowEvent)
        {
            ...
            case FlowEvent.DeliveryMerged:
                ...
                // state projects to. #1885's StreamLogLossDeclared is the same diagnostic-only shape:
                // ExecutionUsageProjector reads it without adding anything to checkpoint state. The four
                // #734 delivery events are the same shape once more, proven the same
                // interleaved-baseline way in this assembly's own test suite (spec/baton.md §2).
                break;
