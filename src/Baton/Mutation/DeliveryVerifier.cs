...
public static class DeliveryVerifier
{
    ...
    public static async Task<DeliveryCheckOutcome> CheckAsync(
        ...
        string ghProgram = "gh")
    {
        var outcome = await CheckCoreAsync(workingDirectory, expectPr, cancellationToken, gitProgram, ghProgram).ConfigureAwait(false);
        ...
        string? workingDirectory, bool expectPr, CancellationToken cancellationToken, string gitProgram, string ghProgram)
    {
        ...
        else if (lsRemoteResult.ExitCode == LsRemoteRefAbsentExitCode)
        {
            ...
            tailLines.Add($"branch-not-pushed: origin has no branch ref for '{branch}' — it has never been pushed.");
        }
        ...
        {
            var pushed = await CheckPushedAsync(gitProgram, workingDirectory, branch, cancellationToken).ConfigureAwait(false);
