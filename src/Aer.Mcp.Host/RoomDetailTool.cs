...
public sealed class RoomDetailTool : IMcpTool
{
    ...
    /// <see cref="Aer.Flow.Status.ExecutionUsageView"/> does, so a retention-swept room still answers.
    /// </summary>
    ...
    /// <see cref="Aer.Flow.Status.ExecutionUsageView"/> uses: the live output directory, then
    ...
    private static async Task<RoomTimelineView?> ReadTimelineAsync(string roomDir, CancellationToken cancellationToken)
    {
        ...
        {
            // AerFlowException covers both a malformed line (FlowEventLogReadException) and a live
