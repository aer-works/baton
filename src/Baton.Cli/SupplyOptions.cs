...
/// exposed on the CLI. Mints a step-less <see cref="Baton.Flow.Mutation.WorkerBinding.NonProcess"/>
/// execution and populates it from <paramref name="SourceFilePath"/> in the same call, so the
/// printed <see cref="Baton.Flow.Domain.ExecutionId"/> is immediately usable as a
...
/// resolve to <see cref="Baton.Flow.Mutation.WorkerBinding.Process"/> (M11's decision of record), so
/// this command constructs the <see cref="Baton.Flow.Mutation.WorkerBinding.NonProcess"/> binding
