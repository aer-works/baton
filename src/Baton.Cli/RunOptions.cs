...
/// <summary>
/// Parsed arguments for <c>baton run</c> (M11 Phase 3): "the CLI is the pump".
/// </summary>
...
/// <c>flow.lock</c>. Running <c>baton run</c> again against the same directory resumes it from the
...
/// to the vendor's reset. Deliberately has no <c>baton run</c> flag and no wire field — the only
...
/// technique <c>baton status --follow</c> already uses — until a *different* process's decision carries
/// the workflow to Terminal, or the caller cancels. It does not reconnect to, or detect, a crashed
/// engine process from an earlier <c>baton run</c> invocation against the same room — that remains an
...
/// <see cref="Baton.Vendors.RoomRegistryStore"/> (spec/baton.md §8's registry) alongside <paramref name="RoomDirectoryPath"/>
...
/// runs — correct for a bare <c>baton run</c>, which has no separate workspace concept; <c>baton dispatch</c>
