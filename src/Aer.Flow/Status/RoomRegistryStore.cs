...
// Lives in Aer.Flow, not Aer.Adapters, for the same reason AerPaths.cs does (see that file's header
// comment): fleet_status (Aer.Mcp.Host) needs to read this registry and deliberately has no
// Aer.Adapters project reference. The namespace stays Aer.Adapters so it sits next to AerPaths,
// DaemonSettingsStore, and the rest of AER's per-machine storage stores it is written from.
...
/// <summary>
/// Reads and writes <see cref="AerPaths.RoomRegistryFile"/> — the spec/baton.md §8 multi-project room registry.
/// </summary>
/// <remarks>
/// <para>
...
public static class RoomRegistryStore
{
    ...
    /// <paramref name="projectRoot"/> are normalized through <see cref="AerPaths.RecordKey"/> so every
        ...
        string roomPath, string projectRoot, string registryFilePath, CancellationToken cancellationToken = default)
    {
        ...
        var entry = new RoomRegistryEntry(
            AerPaths.RecordKey(roomPath), AerPaths.RecordKey(projectRoot), DateTime.UtcNow);
        ...
        string registryFilePath, CancellationToken cancellationToken = default)
    {
        ...
        return Task.Run(
            () =>
            {
                ...
                var byRoom = new Dictionary<string, RoomRegistryEntry>(AerPaths.RecordKeyComparer);
    ...
    /// it is hashed through <see cref="AerPaths.RecordKey"/> first, not the raw string: two spellings of
    /// the same file (forward vs. backward slashes, a different <c>AER_HOME</c> casing) must still hash
    ...
    private static T RunUnderLock<T>(string registryFilePath, Func<T> action)
    {
        var digest = Convert.ToHexString(
            SHA256.HashData(Encoding.UTF8.GetBytes(AerPaths.RecordKey(registryFilePath).ToUpperInvariant())));
        using var mutex = new Mutex(initiallyOwned: false, name: $"Global\\aer-room-registry-{digest}");
