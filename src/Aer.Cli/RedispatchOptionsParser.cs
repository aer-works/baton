...
/// <summary>
/// Parses <c>aer redispatch</c>'s arguments: <c>aer redispatch &lt;room-dir&gt; [--spec &lt;spec-file&gt;]
...
/// <see cref="DispatchOptionsParser"/> documents for <c>aer dispatch</c>. Every malformed invocation is
/// a <see cref="CliArgumentException"/> (CLAUDE.md's error-handling rules).
/// </summary>
public static class RedispatchOptionsParser
{
    /// <summary>The one copy of <c>aer redispatch</c>'s usage line, printed here on error and by <c>Program</c>.</summary>
    public const string Usage =
        "Usage: aer redispatch <room-dir> [--spec <amended-brief>] [--adapter <name>] [--model <name>] "
    ...
    public static RedispatchOptions Parse(IReadOnlyList<string> args)
    {
        ...
        if (parentRoomDirectoryPath is null)
        {
            throw new CliArgumentException(
                ...
                "pass the terminal room directory to redispatch, e.g. aer redispatch <room-dir>.");
        }
        ...
        var freshRoomDirectoryPath = Path.Combine(Aer.Adapters.AerPaths.Rooms, uniqueName);
