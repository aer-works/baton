...
/// #649, and it is why every reviewing template in <c>tools/aer-agy-loop</c> grants write.
/// </para>
/// </remarks>
public sealed class UnsatisfiableOutputContractException : AerFlowException
{
    ...
    public UnsatisfiableOutputContractException(string workerName, IReadOnlyList<string> unwritableOutputs)
        : base(
            ...
            "contract only by writing the artifact into AER_OUTPUT_DIR, so this combination cannot " +
