...
/// All signatures are unsafe-free; the idiomatic managed surface lives in <see cref="BatonTask"/>.
/// </summary>
internal static partial class NativeMethods
{
    ...
    /// The returned handle is freed automatically via <see cref="BatonTaskHandle.ReleaseHandle"/>.
    /// </summary>
    [DllImport(Lib, CallingConvention = CallingConvention.Cdecl)]
    public static extern BatonTaskHandle aer_task_new(
    ...
    /// </summary>
    /// <remarks>
    /// <paramref name="task"/> is typed as the <see cref="BatonTaskHandle"/> SafeHandle itself (not
    ...
    /// </remarks>
    [LibraryImport(Lib)]
    public static partial BatonErrorCode aer_task_with_timeout(BatonTaskHandle task, ulong timeoutMs);

    /// <summary>Enable stdout/stderr capture. Must be called before <see cref="aer_task_run"/>.</summary>
    [LibraryImport(Lib)]
    public static partial BatonErrorCode aer_task_with_capture_output(
        BatonTaskHandle task,
    ...
    /// </remarks>
    [LibraryImport(Lib)]
    public static partial BatonErrorCode aer_task_set_env(
        BatonTaskHandle task,
    ...
    /// </summary>
    [LibraryImport(Lib)]
    public static partial BatonErrorCode aer_task_set_clear_env(
        BatonTaskHandle task,
    ...
    /// <see cref="BatonErrorCode.SpawnFailed"/>.
    /// </summary>
    [LibraryImport(Lib)]
    public static partial BatonErrorCode aer_task_set_cwd(
        BatonTaskHandle task,
    ...
    /// Returns an invalid handle on failure. Freed automatically via <see cref="BatonCancelHandle.ReleaseHandle"/>.
    /// </summary>
    /// <remarks>
    /// <paramref name="task"/> is typed as the <see cref="BatonTaskHandle"/> SafeHandle itself so the CLR
    /// add-refs it for the duration of this call rather than relying on a raw <c>nint</c> plus
    /// <c>DangerousGetHandle</c> (see issue #62).
    /// </remarks>
    [DllImport(Lib, CallingConvention = CallingConvention.Cdecl)]
    public static extern BatonCancelHandle aer_task_make_cancel_handle(BatonTaskHandle task);
    ...
    /// A handle may only be run once; a second call returns <see cref="BatonErrorCode.AlreadyRun"/>.
    /// </summary>
    /// <remarks>
    /// <paramref name="task"/> is typed as the <see cref="BatonTaskHandle"/> SafeHandle itself so the CLR
    ...
    /// </remarks>
    [DllImport(Lib, CallingConvention = CallingConvention.Cdecl)]
    public static extern BatonErrorCode aer_task_run(
        BatonTaskHandle task,
        BatonEventCallback? callback,
        nint userData);
    ...
    /// <see cref="BatonTaskHandle.ReleaseHandle"/>, which by definition already owns the last reference to
    ...
    /// <remarks>
    /// <paramref name="cancel"/> is typed as the <see cref="BatonCancelHandle"/> SafeHandle itself so the
    /// CLR add-refs it for the duration of this call (see issue #62).
    /// </remarks>
    [LibraryImport(Lib)]
    public static partial BatonErrorCode aer_cancel(BatonCancelHandle cancel);

    /// <summary>Free a cancel handle. Safe to call with <see cref="nint.Zero"/>.</summary>
    /// <remarks>
    /// Kept as a raw <c>nint</c> parameter: this is invoked only from
    /// <see cref="BatonCancelHandle.ReleaseHandle"/>, which by definition already owns the last reference to
    /// the handle value and must not go through the SafeHandle itself.
    /// </remarks>
    [LibraryImport(Lib)]
